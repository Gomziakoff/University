...
class SentenceDetailWindow(QDialog):
    ...
    def init_ui(self):
        layout = QVBoxLayout()
        sentence_text = self.sentence_data['sentence_text']
        layout.addWidget(QLabel(f"Предложение: {sentence_text}"))

        try:
            translated_text = self.translate_ru_en(sentence_text)
            print(translated_text)
            blob = TextBlob(translated_text)
            polarity = blob.sentiment.polarity
            subjectivity = blob.sentiment.subjectivity
            tone = "Положительная" if polarity > 0.1 else "Отрицательная" if polarity < -0.1 else "Нейтральная"
            objectivity = "Объективная" if subjectivity < 0.5 else "Субъективная"
            layout.addWidget(QLabel(f"Тональность: {tone} (полярность: {polarity:.2f})"))
            layout.addWidget(QLabel(f"Объективность: {objectivity} (субъективность: {subjectivity:.2f})"))
        except Exception as e:
            layout.addWidget(QLabel("Ошибка анализа тональности: " + str(e)))
