...
class Application(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.add_word_button = ttk.Button(
            self, text="Добавить слово", command=self.add_word
        )
        ...
        self.delete_word_button = ttk.Button(
            self, text="Удалить выбранное слово", command=self.delete_word
        )
        self.delete_word_button.grid(
            row=1, column=0, columnspan=2, padx=5, pady=5, sticky=tk.W + tk.E
        )

        self.generate_forms_button = ttk.Button(
            self, text="Сгенерировать формы", command=self.generate_forms
        )
        self.generate_forms_button.grid(
            row=1, column=2, columnspan=2, padx=5, pady=5, sticky=tk.W + tk.E
        )

        self.word_listbox = tk.Listbox(self, selectmode=tk.SINGLE)
        self.word_listbox.grid(
            row=2,
            column=0,
            columnspan=4,
            padx=5,
    ...
    def add_word(self):
        ...
        if word:
            if word not in [w.word for w in self.words]:
                ...
                word_info = (
                    f"{word} ({', '.join(tag['value'] for tag in word_object.morphy)})"
                )
