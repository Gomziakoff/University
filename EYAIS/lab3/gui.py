...
from levenstein_search_dialog import LevenshteinSearchDialog
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.levenshtein_button = QPushButton("Поиск по Левенштейну")
        self.levenshtein_button.clicked.connect(self.show_levenshtein_dialog)
        ...
        file_buttons_layout.addWidget(self.levenshtein_button)
    ...
    def load_results(self):
        """Загрузка данных из JSON в таблицу и подготовка данных для Левенштейна."""
        ...
        if filename:
            try:
                ...
                self.all_words = []
                for lemma_info in self.original_data:
                    for word_form in lemma_info.get("word_forms", []):
                        word = word_form.get("word")
                        if word:  # Игнорируем пустые значения
                            self.all_words.append(word.lower())  # Приводим к нижнему регистру
                ...
                QMessageBox.critical(
                    self, "Ошибка",
                    f"Некорректный формат данных: отсутствует ключ {str(e)}"
                )
            except Exception as e:
                QMessageBox.critical(
                    self, "Ошибка",
    ...
    def show_levenshtein_dialog(self):
        if not self.all_words:
            QMessageBox.warning(self, "Ошибка", "Сначала обработайте текст!")
            return

        dialog = LevenshteinSearchDialog(self, self.all_words)
        dialog.exec()
